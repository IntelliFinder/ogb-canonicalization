...
def train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs):
    ...
    if net_params['pe_init'] == 'lap_pe':
    ...
    elif net_params['pe_init'] == 'signnet':
        tt = time.time()
        print("[!] -SignNet: Initializing graph positional encoding with Laplacian PE.")
        dataset._add_lap_positional_encodings(net_params['pos_enc_dim'])
        print("[!] Time taken: ", time.time() - tt)
        ...
        dataset._map_ablation(net_params['pos_enc_dim'], use_unique_sign=True, use_unique_basis=True, use_eig_val=False)
        print("[!] Time taken: ", time.time() - tt)
    elif net_params['pe_init'] == 'oap':
        tt = time.time()
        print("[!] -OAP: Initializing graph positional encoding with OAP.")
        dataset._add_oap_positional_encodings(net_params['pos_enc_dim'])
        print("[!] Time taken: ", time.time() - tt)
    elif net_params['pe_init'] == 'oap_ablation':
        tt = time.time()
        print("[!] -FA: Initializing graph positional encoding with Laplacian PE.")
        dataset._oap_ablation(net_params['pos_enc_dim'])
        print("[!] Time taken: ", time.time() - tt)
    ...
    ___, __, g_outs_test = evaluate_network(model, device, test_loader, epoch, evaluator, dataset.name)
    del ___
    del __
...
def main():
    # os.chdir('Graph PE/MaskedGIN as embedding_p')
    ...
    parser.add_argument('--phi_out_dim', help="Please give a value for lap lspe")
    ...
    for i in range(4):
        params['seed'] = seeds[i]
        accs[i] = train_val_pipeline(MODEL_NAME, dataset, params, net_params, dirs)
        print()
