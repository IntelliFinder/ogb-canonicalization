...
class GraphiTNet(nn.Module):
    def __init__(self, net_params):
        ...
        if self.pe_init == 'rand_walk' or self.use_lspe:
            ...
            self.layers = nn.ModuleList(
                [GraphiT_GT_LSPE_Layer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph, dropout,
                                       self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE) for _ in
                 range(GT_layers - 1)])
            self.layers.append(GraphiT_GT_LSPE_Layer(gamma, GT_hidden_dim, GT_out_dim, GT_n_heads, full_graph, dropout,
                                                     ...
                                                     self.adaptive_edge_PE))
        else:
            # NoPE or LapPE or MAP or OAP
            self.layers = nn.ModuleList(
                [GraphiT_GT_Layer(gamma, GT_hidden_dim, GT_hidden_dim, GT_n_heads, full_graph, dropout,
                                  self.layer_norm, self.batch_norm, self.residual, self.adaptive_edge_PE) for _ in
                 range(GT_layers - 1)])
    ...
    def forward(self, g, h, p, e, snorm_n):
        ...
        if self.pe_init == 'rand_walk' or self.use_lapeig_loss or self.use_lspe:
            p = self.p_out(p)
            g.ndata['p'] = p
        if self.pe_init == 'rand_walk' or self.use_lapeig_loss:
