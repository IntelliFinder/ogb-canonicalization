...
class CSL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for sample in self.adj_list:
            ...
            from dgl.transforms import RemoveSelfLoop
...
def use_positional_encoding(g, pos_enc_dim, use_unique_sign=True, use_unique_basis=True, use_eig_val=True):
    """
...
class CSLDataset(torch.utils.data.Dataset):
    ...
    def _add_positional_encodings(self, pos_enc_dim):
        ...
        for split_num in range(5):
            ...
            self.train[split_num].lists = (self.train[split_num].graph_lists, self.train[split_num].graph_labels)
            self.val[split_num].graph_lists = [positional_encoding(g, pos_enc_dim) for g in
                                               self.val[split_num].graph_lists]
            self.val[split_num].lists = (self.val[split_num].graph_lists, self.val[split_num].graph_labels)
            self.test[split_num].graph_lists = [positional_encoding(g, pos_enc_dim) for g in
                                                self.test[split_num].graph_lists]
            self.test[split_num].lists = (self.test[split_num].graph_lists, self.test[split_num].graph_labels)
    ...
    def _add_use_positional_encodings(self, pos_enc_dim):
        ...
        if osp.exists(cache_path):
            with open(cache_path, "rb") as f:
                cache = pickle.load(f)
                for split_num in range(5):
                    self.train[split_num].graph_lists, self.val[split_num].graph_lists, self.test[split_num].graph_lists = cache[split_num]
                    self.train[split_num].lists = (self.train[split_num].graph_lists, self.train[split_num].graph_labels)
                    self.val[split_num].lists = (self.val[split_num].graph_lists, self.val[split_num].graph_labels)
                    self.test[split_num].lists = (self.test[split_num].graph_lists, self.test[split_num].graph_labels)
        else:
            cache = []
            for split_num in range(5):
                self.train[split_num].graph_lists = [use_positional_encoding(g, pos_enc_dim) for g in
                                                     self.train[split_num].graph_lists]
                self.train[split_num].lists = (self.train[split_num].graph_lists, self.train[split_num].graph_labels)
                self.val[split_num].graph_lists = [use_positional_encoding(g, pos_enc_dim) for g in
                                                   self.val[split_num].graph_lists]
                self.val[split_num].lists = (self.val[split_num].graph_lists, self.val[split_num].graph_labels)
                self.test[split_num].graph_lists = [use_positional_encoding(g, pos_enc_dim) for g in
                                                    self.test[split_num].graph_lists]
                self.test[split_num].lists = (self.test[split_num].graph_lists, self.test[split_num].graph_labels)
                cache.append((self.train[split_num].graph_lists, self.val[split_num].graph_lists,
                              self.test[split_num].graph_lists))
            with open(cache_path, "wb") as f:
                pickle.dump(cache, f)

    def _use_ablation(self, pos_enc_dim, use_unique_sign=True, use_unique_basis=True, use_eig_val=True):
        torch.set_default_dtype(torch.float64)
        for split_num in range(5):
            self.train[split_num].graph_lists = [
                use_positional_encoding(g, pos_enc_dim, use_unique_sign, use_unique_basis,
                                        use_eig_val) for g in self.train[split_num].graph_lists]
            self.val[split_num].graph_lists = [
                use_positional_encoding(g, pos_enc_dim, use_unique_sign, use_unique_basis,
                                        use_eig_val) for g in self.val[split_num].graph_lists]
            self.test[split_num].graph_lists = [
                use_positional_encoding(g, pos_enc_dim, use_unique_sign, use_unique_basis,
                                        use_eig_val) for g in self.test[split_num].graph_lists]
            self.train[split_num].lists = (self.train[split_num].graph_lists, self.train[split_num].graph_labels)
            self.val[split_num].lists = (self.val[split_num].graph_lists, self.val[split_num].graph_labels)
            self.test[split_num].lists = (self.test[split_num].graph_lists, self.test[split_num].graph_labels)
