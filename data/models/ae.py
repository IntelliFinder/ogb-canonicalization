...
import torch
from data.models.gcl import GCL, GCL_rf, E_GCL, E_GCL_vel
...
class MC_AE_EGNN(AE_parent):
    def __init__(self, in_node_nf, in_edge_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,
        ...
        for i in range(1, n_layers - 1):
            self.add_module("gcl_%d" % i, E_GCL_vel(self.hidden_node_nf, self.hidden_node_nf, self.hidden_edge_nf, self.hidden_node_nf, self.hidden_coord_nf, edges_in_d=in_edge_nf, act_fn=act_fn, coords_weight=coords_weight, recurrent=recurrent, norm_diff=norm_diff, tanh=tanh, num_vectors_in=num_vectors, num_vectors_out=num_vectors)) #fix EGCL_vel with no velocity and run
