...
class E_GCL(nn.Module):
    ...
    def edge_model(self, source, target, radial, edge_attr):
        if edge_attr is None:  # Unused.
            out = torch.cat([source, target, radial.to(source.device)], dim=1)
        else:
    ...
    def coord_model(self, coord, edge_index, coord_diff, radial, edge_feat):
        ...
        trans = torch.einsum('bij,bci->bcj', coord_matrix.to(coord.device), coord_diff.to(coord.device))
        trans = torch.clamp(trans, min=-100, max=100) #This is never activated but just in case it case it explosed it may save the train
        agg = unsorted_segment_sum_vec(trans, row, num_segments=coord.size(0))
        if self.last_layer:
...
class GCL_rf_vel(nn.Module):
    ...
    def node_model(self, x, edge_index, edge_m):
        row, col = edge_index
        agg = unsorted_segment_sum(edge_m, row, num_segments=x.size(0))
...
def unsorted_segment_sum(data, segment_ids, num_segments):
    ...
    result.scatter_add_(0, segment_ids.to('cuda'), data.to('cuda'))
    return result
...
def unsorted_segment_sum_vec(data, segment_ids, num_segments):
    ...
    result = data.new_full(result_shape, 0)  # Init empty result tensor.
    result.scatter_add_(0, segment_ids.to(data.device), data.to(data.device))
    return result 

def unsorted_segment_mean(data, segment_ids, num_segments):
    result_shape = (num_segments, data.size(1), data.size(2))
    segment_ids = segment_ids.unsqueeze(-1).unsqueeze(-1).to(data.device)
    segment_ids = segment_ids.expand(-1, data.size(1), data.size(2))
    result = data.new_full(result_shape, 0)  # Init empty result tensor.
